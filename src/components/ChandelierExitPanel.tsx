...
export const ChandelierExitPanel: React.FC<ChandelierExitPanelProps> = ({ data, activeTimeframe, language }) => {
    ...
    const trendLabel = ce.trend === 'BULLISH' ? t('trend_up')
        : ce.trend === 'BEARISH' ? t('trend_down')
        : t('neutral');
    ...
    return (
        <div className="card">
            ...
            <div className="p-4 space-y-3">
                {/* Trend Badge */}
                <div className={`flex items-center justify-between p-3 rounded-[1px] border ${trendBg}`}>
                    <div className="flex items-center gap-2">
                        {trendIcon}
                        <span className={`text-sm font-bold ${trendColor}`}>{trendLabel}</span>
                    </div>
                    <span className="text-xs text-[var(--color-text-secondary)]">
                        {t('period')}: 1 | ATR × 1.85 (HA RMA)
                    </span>
                </div>

                {/* Signal Alerts */}
                {ce.buySignal && (
                    <div className="flex items-center gap-2 p-3 bg-green-500/15 border border-green-500/40 rounded-[1px] animate-pulse">
                        <ArrowUpRight size={18} className="text-green-400" />
                        <div>
                            <span className="text-green-400 font-bold text-sm">🔔 {t('buy_signal')}</span>
                            <p className="text-xs text-green-300/70 mt-0.5">{t('price_above_short')}</p>
                        </div>
                    </div>
                )}
                {ce.sellSignal && (
                    <div className="flex items-center gap-2 p-3 bg-red-500/15 border border-red-500/40 rounded-[1px] animate-pulse">
                        <ArrowDownRight size={18} className="text-red-400" />
                        <div>
                            <span className="text-red-400 font-bold text-sm">🔔 {t('sell_signal')}</span>
                            <p className="text-xs text-red-300/70 mt-0.5">{t('price_below_long')}</p>
                        </div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="p-3 bg-[var(--color-bg-tertiary)] rounded-[1px]">
                        ...
                        <div className={`text-[10px] mt-1 ${distToExitLong > 0 ? 'text-green-400' : 'text-red-400'}`}>
                            {distToExitLong > 0 ? '↑' : '↓'} {Math.abs(distToExitLong).toFixed(2)}% {t('from_price')}
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                    <div className="text-center p-2 bg-[var(--color-bg-tertiary)] rounded-[1px]">
                    ...
                    <div className="text-center p-2 bg-[var(--color-bg-tertiary)] rounded-[1px]">
                        <div className="text-[10px] text-[var(--color-text-secondary)] uppercase">{t('highest_high')}</div>
                        <div className="text-sm font-bold text-green-400">${formatPrice(ce.highestHigh)}</div>
                    </div>
                    <div className="text-center p-2 bg-[var(--color-bg-tertiary)] rounded-[1px]">
                        <div className="text-[10px] text-[var(--color-text-secondary)] uppercase">{t('lowest_low')}</div>
                ...
                <div className="text-[10px] text-[var(--color-text-secondary)] leading-relaxed border-t border-[var(--color-border)] pt-2">
                    <span className="text-green-400">■</span> {t('exit_long_trailing')} = {t('highest_high')} HA(1) − ATR × 1.85 • 
                    <span className="text-red-400"> ■</span> {t('exit_short_trailing')} = {t('lowest_low')} HA(1) + ATR × 1.85
                </div>
            </div>
        </div>
    );

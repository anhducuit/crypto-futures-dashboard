        ...
        <div className="card bg-gradient-to-br from-[var(--color-bg-secondary)] to-[#1a1a2e] border-[var(--color-border)] overflow-hidden">
            <div className="card-header border-b border-[var(--color-border)] pb-3 mb-4">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold tracking-wider uppercase">Divergence Analysis</span>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-white/5 border border-white/5">
                    <span className="text-xs text-[var(--color-text-secondary)]">RSI Current</span>
                ...
                <div className={`flex flex-col items-center justify-center p-6 rounded-2xl border-2 border-dashed transition-all duration-500 ${isBullish ? 'bg-green-500/10 border-green-500/50 shadow-[0_0_20px_rgba(34,197,94,0.2)]' :
                    ...
                    {isBullish ? (
                        <>
                            ...
                            <span className="text-lg font-black text-green-500 tracking-tighter">BULLISH DIVERGENCE</span>
                            <p className="text-[10px] text-green-400/70 mt-1 uppercase font-bold">Tín hiệu đảo chiều TĂNG</p>
                        </>
                    ) : isBearish ? (
                        <>
                            <TrendingDown size={48} className="text-red-500 mb-3 animate-bounce" />
                            <span className="text-lg font-black text-red-500 tracking-tighter">BEARISH DIVERGENCE</span>
                            <p className="text-[10px] text-red-400/70 mt-1 uppercase font-bold">Tín hiệu đảo chiều GIẢM</p>
                        </>
                    ) : (
                        <>
                            <AlertCircle size={40} className="text-[var(--color-text-secondary)] mb-3 opacity-30" />
                            <span className="text-sm font-bold text-[var(--color-text-secondary)] opacity-50 uppercase tracking-widest">No Divergence</span>
                        </>
                    )}
                </div>

                {/* Quick Guide */}
                <div className="p-3 rounded-lg bg-black/20 text-[10px] text-[var(--color-text-secondary)] leading-relaxed italic">
                    * Phân kỳ được tính dựa trên 50 cây nến gần nhất, so sánh đỉnh/đáy giá với đỉnh/đáy RSI.
                </div>
            </div>
        </div>
    );
